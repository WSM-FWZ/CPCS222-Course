/* ============================================================
   ...
   json_loader.js  —  Data store (Phase 2)
...
(function (global) {
  ...
  var _LESSONS = {
    ...
    '1.1': {
      ...
      story: `Student: "Professor, my friend says 'x + 1 = 2' is a proposition. I disagree. Who's right?"

Professor: "You are. A proposition must be a declarative sentence that is definitely either true or false. 'x + 1 = 2' depends on x — it's an open sentence, not yet a proposition. The moment we plug in a value, say x = 1, it becomes the proposition '1 + 1 = 2', which is true."

Student: "So commands and questions don't count?"

Professor: "Right. 'Close the door!' is a command, 'What time is it?' is a question. Neither is true or false. Now — what truth value does 'The Moon is made of green cheese' have?"

Student: "False — but it's still a proposition, because it CAN be evaluated."

Professor: "Exactly. Truth value, not truthfulness, is what matters."`,
...
Inverse (¬p → ¬q): "If it does not snow, then I will not stay home."`
    ...
    '1.2': {
      ...
      story: `Student: "My intern team got a spec that said: 'The reply is sent unless the file system is full,' AND 'The reply is not sent.' We spent two days arguing whether the file system has to be full."

Professor: "Translate them. Let r = 'reply is sent', f = 'file system is full'. 'Reply sent unless full' becomes ¬f → r — equivalently r ∨ f. Combined with ¬r, what does logic force?"

Student: "r ∨ f and ¬r both true... so f must be true. The file system MUST be full."

Professor: "You just resolved a two-day argument with one truth table. That's the value of formalization."`,
...
No row satisfies all four — UNSATISFIABLE (a contradiction).`
      ...
      visual: {
        type: 'logic-flow',
        idea: 'An English-to-logic translator panel: highlight trigger phrases (only if, whenever, unless, necessary, sufficient) and show the matching connective.',
    ...
    '1.3': {
      ...
      story: `Student: "Professor, my code has the condition !(a && b) || (a && b). VS Code says it's always true. Why?"

Professor: "Let p = a∧b. The expression is ¬p ∨ p — that's the law of the excluded middle. Always true, regardless of p."

Student: "So I could replace the whole condition with true?"

Professor: "Yes — and a modern compiler would. But knowing WHY it can do that is what we're learning. We proved it through identity laws, not by simulation."

Student: "What if the code is (p ∧ q) ∨ (p ∧ ¬q)?"

Professor: "Distribute: p ∧ (q ∨ ¬q) = p ∧ T = p. The q dropped out completely. That's a real optimization."`,
...
≡ T.                   domination`
      ...
      quiz: [
        { q: 'Which is logically equivalent to ¬(p → q)?', choices: ['¬p → ¬q', 'p ∧ ¬q', '¬p ∨ q', 'q → p'], answer: 'p ∧ ¬q', explanation: 'p → q ≡ ¬p ∨ q; negating gives ¬(¬p ∨ q) ≡ p ∧ ¬q.' },
        { q: '¬(p ∨ q) is equivalent to:', choices: ['¬p ∨ ¬q', '¬p ∧ ¬q', 'p ∧ q', 'p ↔ q'], answer: '¬p ∧ ¬q', explanation: "De Morgan's second law." },
        { q: 'Simplify (p ∧ T) ∨ (q ∧ F).', choices: ['p', 'q', 'p ∨ q', 'T'], answer: 'p', explanation: 'p ∧ T ≡ p; q ∧ F ≡ F; p ∨ F ≡ p.' },
        { q: 'Which law justifies (p ∧ q) ∨ p ≡ p?', choices: ['Identity', 'Domination', 'Absorption', 'Idempotent'], answer: 'Absorption', explanation: 'Absorption laws: p ∨ (p ∧ q) ≡ p and p ∧ (p ∨ q) ≡ p.' },
        { q: '(p → q) ∨ (p → ¬q) is:', choices: ['A contradiction', 'A tautology', 'Equivalent to p', 'Equivalent to q'], answer: 'A tautology', explanation: '≡ (¬p ∨ q) ∨ (¬p ∨ ¬q) ≡ ¬p ∨ (q ∨ ¬q) ≡ ¬p ∨ T ≡ T.' }
      ],
      ...
      visual: {
        type: 'logic-flow',
        idea: 'A Law Card Deck: click a law card and a sub-expression to apply that rewrite.',
    ...
    '1.4': {
      ...
      story: `Student: "How do I say 'every man is mortal' in propositional logic?"

Professor: "You can't — and that's the point. Propositional logic only talks about fixed facts. Once we say 'every', we're ranging over a domain of objects. We need predicates and quantifiers."

Student: "OK — so M(x) means 'x is mortal'. How do I say 'every man'?"

Professor: "Add Man(x). Now: ∀x (Man(x) → M(x)). For every x, if x is a man, then x is mortal. The implication does the filtering — non-men make the implication vacuously true."

Student: "And 'some man is brave'?"

Professor: "With existential, you use ∧, not →. ∃x (Man(x) ∧ B(x)). Otherwise ∃x (Man(x) → B(x)) would be true if even one non-man exists, since the implication holds vacuously."`,
...
Intuition: "not every x is P" = "some x is not P".`,
    ...
    '1.5': {
      ...
      story: `Student: "Are ∀x ∃y P(x,y) and ∃y ∀x P(x,y) the same?"

Professor: "Almost never. Take P(x,y) = 'x + y = 0' over the reals. The first says: for every real x, some y zeros it out — TRUE, y = -x. The second says: there's ONE y that zeros out every x — FALSE, no single number is the inverse of every real."

Student: "So the swap matters when ∀ comes first vs second?"

Professor: "Always pay attention. ∀∃ allows y to depend on x. ∃∀ forces a single y that works uniformly. The second is strictly stronger — it implies the first, but not the other way around."

Student: "Like in calculus — continuity vs uniform continuity?"

Professor: "Exactly that. The δ that depends on x vs the δ that doesn't. Quantifier order is the whole difference."`,
...
NEGATING NESTED QUANTIFIERS — push the negation inward:
  ...
  ¬∃x ∀y P(x,y)  ≡  ∀x ¬∀y P(x,y)  ≡  ∀x ∃y ¬P(x,y)`,
      ...
      visual: {
        type: 'simulation',
        idea: 'A 2-D grid: x-axis one variable, y-axis another. Buttons toggle ∀∀, ∀∃, ∃∀, ∃∃.',
    ...
    '1.6': {
      ...
      story: `Student: "How do I PROVE 'It rained' from 'If it didn't rain, the race is held' and 'The trophy wasn't awarded' and 'If the race is held, the trophy is awarded'?"

Professor: "Chain rules. Race held → trophy awarded; trophy wasn't awarded — that's Modus Tollens, so race not held. Then 'not rained → race held' contraposes to 'race not held → it rained'. Apply Modus Ponens."

Student: "So I built a chain of rules instead of one big truth table."

Professor: "Yes — and for n premises with k variables, the truth-table approach is exponential in k. The rule-of-inference approach is linear in the number of steps. That's why proof assistants are practical and brute-force is not."`,
...
FALLACIES TO AVOID:
  ...
  Denying the Antecedent:   from p → q and ¬p, infer ¬q — INVALID.`,
      ...
      visual: {
        type: 'logic-flow',
        idea: 'A Proof Tree Builder: display premises as leaf cards and connect them via labeled edges representing inference rules.',
    ...
    '1.7': {
      ...
      story: `Student: "How do I prove 'if n is odd, then n² is odd'?"

Professor: "Direct proof. Assume the hypothesis: n is odd, so n = 2k + 1 for some integer k. Square: n² = 4k² + 4k + 1 = 2(2k² + 2k) + 1, which has the form 2r + 1, so n² is odd. ∎"

Student: "What about 'if n² is odd, then n is odd'?"

Professor: "Direct is awkward — you'd have to factor n² = 2k+1, which is messy. Try CONTRAPOSITION: assume n is even, show n² is even. n = 2k, so n² = 4k² = 2(2k²), even. By contraposition, n² odd → n odd."

Student: "And √2 is irrational?"

Professor: "That one needs CONTRADICTION. Suppose √2 = a/b in lowest terms; derive that a and b are both even — contradicting 'lowest terms'."`,
...
DEFINITIONS:
  ...
  r is RATIONAL iff ∃p, q ∈ ℤ with q ≠ 0, r = p/q.`,
      ...
      visual: {
        type: 'logic-flow',
        idea: 'A Proof Method Selector diagram: three branches descend — Direct, Contrapositive, Contradiction.',
    ...
    '1.8': {
      ...
      story: `Student: "I need to show 'for every integer n, |n| ≥ 0'. But absolute value is defined piecewise — how do I write one proof?"

Professor: "PROOF BY CASES. Handle each piece of the definition separately. Case 1: n ≥ 0, then |n| = n ≥ 0. Case 2: n < 0, then |n| = −n > 0. Both cases cover every integer, so done."

Student: "And how do I show 'there exist irrationals x, y with x^y rational'?"

Professor: "NONCONSTRUCTIVE EXISTENCE. Look at √2^√2. If it's rational, take x = y = √2. If it's irrational, take x = √2^√2 and y = √2; then x^y = (√2^√2)^√2 = √2^2 = 2, rational. Either way, such x and y exist — without ever telling you which case is the real one."

Student: "So I proved existence without producing the actual numbers?"

Professor: "That's the whole point. Constructive proofs build the witness; nonconstructive proofs show that one must exist."`,
...
FORWARD vs BACKWARD REASONING:
  ...
  • BACKWARD: start from conclusion, find a sufficient condition, then a sufficient condition for that.`,
      ...
      visual: {
        type: 'logic-flow',
        idea: 'A Proof Strategy Decision Tree: starting from the form of the claim, the diagram branches into the recommended technique.',
  ...
  function _validate() {
    ...
    sections.forEach(function(s) {
      var l = _LESSONS[s];
      if (!l.title)    { console.warn('JsonLoader: missing title for', s); }
